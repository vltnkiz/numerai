...
Plain per-era rank correlation between a prediction and the target. Numerai prices nothing on it. It survives in this project for one reason: the cross-run history log has recorded `mean_corr`, `sharpe` and `smart_sharpe` in this vocabulary since [issue #68](https://github.com/vltnkiz/numerai/issues/68), and those entries stay comparable for a year after the live run moves to the paid metrics. On the same production run it reads 0.0164 where **Numerai CORR** reads 0.0104 — it ranks differently *and* has been taken on the raw blend rather than the neutralized one, so the gap is a difference of metric **and** of artifact (see [issue #96](https://github.com/vltnkiz/numerai/issues/96)).
_Avoid_: comparing a Spearman number against a Numerai CORR number, and measuring anything new on Spearman at all.
