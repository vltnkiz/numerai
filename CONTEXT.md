...
**Numerai CORR**:
The correlation Numerai pays on, measured per era: prediction and payout target are each ranked and gaussianized, the prediction is raised to the power 1.5, and the two are correlated. One of the two priced halves of **live payout**; MMC is the other, and carries the heavier weight. Taken from `numerai-tools`, Numerai's own reference implementation, rather than reimplemented.
...
**Paid metrics**:
The vocabulary the harness ranks strategies in, measured on validation: **Numerai CORR**; MMC, what a prediction adds to the target's correlation beyond what the meta model's own prediction already accounts for; and, in the harness only, `validation_payout_proxy`, `0.75·CORR + 2.25·MMC` with both halves taken over the **meta-model window**. The proxy is the formula rounds up to 1342 paid on, kept as a ranking column; it predicts nothing about what a round pays. The gate reads only CORR. The live run records CORR and MMC and no payout (see `docs/adr/0002-the-live-run-gates-on-one-number-and-records-the-rest.md` and `docs/adr/0003-live-scores-come-from-numerai.md`).
_Avoid_: "score" or "validation score" for any one of them, and "payout" for the proxy. Say which metric.

**Live scores** (`live_scores.jsonl`):
Numerai's own per-round scores for each submission slot, fetched from Numerai and never computed here: one row per (model, round), provisional until the round resolves and frozen after. A round's `payout_score` uses the payout multipliers Numerai lists on that round; its `reference_payout_score` prices it by the current formula (`3·corr60 + 15·mmc60`, rounds from 1343), which is what the headline averages, over resolved rounds only. Neither includes stake, payout factor or clipping. zemir_01 is unstaked, so it pays 0 NMR whatever it scores (see `docs/adr/0003-live-scores-come-from-numerai.md`).
_Avoid_: comparing any validation number with a live score, or averaging `payout_score` across rounds priced by different formulas.
...
_Avoid_: comparing MMC or the payout proxy between two measurements taken over different windows. A longer window is not a better score, only a different one; check `mmc_eras` first.
